...
function renderDb(){const list=dbRows();
  ...
  app.querySelectorAll('[data-dbedit]').forEach(el=>{el.onchange=e=>{el.title=e.target.value;dbEdit(Number(el.dataset.n),el.dataset.dbedit,e.target.value)}});
}
function dbRowHtml(r){return `<tr class="${r.unresolved?'unresolved':''}${dbSel.has(r.number)?' picked':''}"><td class="db-check"><input type="checkbox" data-dbsel="${r.number}" ${dbSel.has(r.number)?'checked':''}></td>${DB_COLS.map(c=>{
  ...
  return `<td class="${c.cls||''}"><input class="cell-input${c.cls?' price-input money':''}" data-dbedit="${c.k}" data-n="${r.number}" value="${esc(r[c.k]??'')}" title="${esc(r[c.k]??'')}"></td>`}).join('')}</tr>`}
