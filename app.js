...
].map(r=>({number:r[0],artist:r[1],release:r[2],label:r[3],catno:r[4],country:r[5],year:r[6],tracks:r[7],unresolved:r[8],discogsId:'',media:'VG+',sleeve:'VG+',numForSale:'',lowestPrice:'',currency:'',askingPrice:'',discogsUrl:'',marketplaceStatus:'Pending'}));
...
<div class="toolbar"><input id="q" placeholder="Search artist, release, label, catalog number…" value="${esc(query)}"><select id="filter"><option value="all">All records</option><option value="unresolved">Needs verification</option><option value="linked">Discogs linked</option><option value="priced">Marketplace priced</option></select><button class="btn primary" id="syncAll">${syncing?'Syncing…':'Refresh all from Discogs'}</button><button class="btn" id="calcAll">Calculate all</button><button class="btn" id="xls">Export Excel</button><button class="btn" id="json">Backup JSON</button><button class="btn" id="print">Print / PDF</button><button class="btn" id="reset">Reset catalog</button></div>
...
async function syncAll(){if(syncing)return;
  ...
  const list=rows.slice();let done=0,linked=0,pricedN=0,noMatch=0,failed=0,skipped=0;
  for(const r of list){
    if(cancelSync)break;
